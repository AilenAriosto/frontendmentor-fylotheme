import "./assets/scss/styles.scss";
import "./assets/scss/styles_tablet.scss";
import "./assets/scss/styles_desktop.scss";

import "./assets/images/bg-curvy-desktop.svg";
import "./assets/images/bg-curvy-mobile.svg";
import "./assets/images/bg-quotes.png";
import "./assets/images/facebook.svg";
import "./assets/images/favicon-32x32.png";

import "./assets/images/icon-access-anywhere.svg";
import "./assets/images/icon-any-file.svg";
import "./assets/images/icon-arrow.svg";
import "./assets/images/icon-collaboration.svg";
import "./assets/images/icon-email.svg";

import "./assets/images/icon-location.svg";
import "./assets/images/icon-phone.svg";
import "./assets/images/icon-security.svg";
import "./assets/images/illustration-intro.png";
import "./assets/images/illustration-stay-productive.png";
import "./assets/images/instagram.svg";
import "./assets/images/logo.svg";
import "./assets/images/profile-1.jpg";
import "./assets/images/profile-2.jpg";
import "./assets/images/profile-3.jpg";
import "./assets/images/twitter.svg";

import "./assets/js/brands.js";
import "./components/sections/sectionNewsletter/sectionNewsletter.js";